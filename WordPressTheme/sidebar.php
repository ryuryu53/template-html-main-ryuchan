  ...
  <div class="column-aside__container">
    <div class="column-aside__article">
      ...
      <div class="column-aside__items article-cards">
        ...
        <!-- wp_reset_postdata()：ループで使用された投稿データをリセットして、WordPressの通常の投稿データに戻す（サブループで使った投稿データは破棄され、メインクエリの投稿データが復元される ⇒ より正確に表現すると「メインクエリの（最後に処理した）投稿データに戻す」･･･ single.phpの場合 → 本文記事に戻る、home.phpの場合 → 記事一覧の最後の投稿に戻る） -->
        <!-- wp_reset_postdata()がやっていること：グローバル変数 $postを、サブループを始める前の状態 に戻す。つまり「そのサブループの前に参照していた投稿」に戻すだけ。「元の $postに戻す」のが本質 ･･･ single.phpの場合 → サブループ前は「本文記事の $post」なので、それに戻る、home.php の場合 → サブループ前は「メインループの最後のブログ記事の $post」なので、それに戻る ⇒ どちらも「メインクエリ（本文や記事一覧）で扱っていた投稿データ」に戻る、と言える -->
    ...
    <div class="column-aside__campaign">
      ...
      <div class="column-aside__btn-2">
        ...
        <a href="<?php echo esc_url( get_post_type_archive_link( 'campaign' ) ); ?>" class="button" aria-label="キャンペーン一覧を見る">
          <span class="button__text">View&nbsp;more</span>
        </a>
      </div>
    </div>
    ...
    <div class="column-aside__archive">
      ...
      <div class="column-aside__items">
        ...
        // SELECT：データベースから特定の情報を取得する  DISTINCT：重複を除いて、一意な結果を取得する → 不要、「GROUP BY year, month」を書いた時点で、結果セットは「年 × 月」の組み合わせごとに 1行にまとめられる。つまり、year, monthの組み合わせは必ずユニークになる（25.10.3）  YEAR(post_date)：投稿の日付（post_date）から年だけを抽出する関数
        ...
        <div class="column-aside__item-archive archive-toggle js-archive-toggle">
          <h3 class="archive-toggle__year-title<?php echo esc_attr( $is_open_class ); ?> js-archive-toggle-title">
